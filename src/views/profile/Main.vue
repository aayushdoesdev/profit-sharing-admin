...
function checkForm() {
  ...
  if (!formData.value.name.trim() || !formData.value.email.trim() || !formData.value.mobile.trim()) {
    return true;
  }
...
const handleChangePassword = async () => {
  if (changePasswordFormData.value.old_password && 
      changePasswordFormData.value.new_password && 
      changePasswordFormData.value.confirm_password === changePasswordFormData.value.new_password) {
...
<template>
  <main class="bg-white p-4 m-4 rounded-md flex flex-col md:flex-row items-start gap-4">
    <div class="p-4 flex flex-col items-center w-full md:w-[20%] space-y-4">

      <div class="text-center">
        <div class="relative">
          <div class="w-[100px] h-[100px] bg-slate-500 rounded-full overflow-hidden">
            <img 
    ...
    <hr class="border border-dashed border-black border-opacity-10 w-full md:hidden" />
    ...
    <div class="w-full md:w-[80%] space-y-4">
      <div class="space-y-2">
        ...
        <div class="grid grid-cols-2 nrml-text gap-4 md:gap-8">
          <div class="space-y-1">
            <p>User Name</p>
            <input v-model="formData.name" type="text" :disabled="disableUserForm" class="w-full bg-transparent border border-opacity-20 rounded-md outline-none px-4 py-2
             disabled:opacity-50 disabled:cursor-not-allowed disabled:text-gray-900" />
          </div>
          <div class="space-y-1">
            <p>Email</p>
            <input v-model="formData.email" type="text" disabled class="w-full bg-transparent border border-opacity-20 rounded-md outline-none px-4 py-2
             disabled:opacity-50 disabled:cursor-not-allowed disabled:text-gray-900" />
          </div>
          <div class="space-y-1">
            <p>Number</p>
            <input v-model="formData.mobile" type="text" disabled class="w-full bg-transparent border border-opacity-20 rounded-md outline-none px-4 py-2
             disabled:opacity-50 disabled:cursor-not-allowed disabled:text-gray-900" />
          </div>
        </div>
      </div>
